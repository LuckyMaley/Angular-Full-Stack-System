...
export class WomensComponent implements OnInit, AfterViewChecked {
  products: Product[] = [];
  reviews: Review[] = [];
  categories: Category[] = [];
  ...
  constructor(
    ...
    private crudService: CrudService, private authService: AuthService, private router: Router
  ) {}
  ...
  fetchData(): void {
    ...
    this.homeService.getProducts().subscribe(
      (data: Product[]) => {
        this.products = data.filter((p) => p.type == 'Women');
      },
      (error) => {
