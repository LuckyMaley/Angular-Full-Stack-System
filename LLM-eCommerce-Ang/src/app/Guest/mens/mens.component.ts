...
export class MensComponent implements OnInit, AfterViewChecked {
  products: Product[] = [];
  reviews: Review[] = [];
  categories: Category[] = [];
  errorMessage: string | null = null;

  constructor(private homeService: HomeService, private cartService: CartService, private authService: AuthService, private router: Router, private crudService: CrudService) {}
  ...
  fetchData(): void {
    this.homeService.getCategories().subscribe(
      ...
      this.homeService.getProducts().subscribe((data: Product[]) => {
        this.products = data.filter(p => p.type == 'Men');
      },
      error => {
